        ...
        <div id="frmBusqueda">
            <table class="fuente8" width="100%" cellspacing="0" cellpadding="5" border="0">
                ...
                <tr>
                    ...
                    <td colspan="3" style="color: #FF3770; margin: top 40px;">
                    Tipo de Operación (NUBECONT) &nbsp;&nbsp;&nbsp;
                    <select name="tipo_operacionN" id="tipo_operacionN">
                        <option value="">Seleccione un tipo de operación</option>
                        <option value="TV001">TV001 - VENTA DE MERCADERÍAS (PRODUCTOS)</option>
                        <option value="TV002">TV002 - PRESTACIÓN DE SERVICIOS</option>
                        <option value="TV003">TV003 - VENTA DE PRODUCTOS TERMINADOS (MANUFACTURADOS)</option>
                        <option value="TV004">TV004 - VENTA DE SUBPRODUCTOS</option>
                    </select>
                ...
                <?php } else { ?>
                    <tr>                   
                        ...
                        <td colspan="2">
                            <input type="text" class="cajaMinima" id="descuento" name="descuento" size="3" value="<?php echo $descuento; ?>" onKeyPress="return numbersonly(this,event,'.');" onblur="calcular_totales_tempdetalle();">
                            &nbsp;
                            <span style="display: <?php if(!$usa_adelanto == 1) echo 'none' ?>;" id="box-adelantos">
                                Adelanto : 
                                &nbsp;
                    ...
                    <td rowspan="12" align="left" valign="top">
                        <table width="100%" border="0" align="right" cellpadding="3" cellspacing="0" class="fuente8" >
                            ...
                            <tr>
                                ...
                                <td style="width: 10%;">
                                    <input id="montoFP_default" type="text" name="montoFP_default" value="<?php echo $montoFP ?>" class="form-control w-porc-50 h-1" placeholder="Monto" style="width: 90px">
                                    <input type="hidden" id="montoFP_default2" name="montoFP_default2" value="<?php echo $montoFP ?>">
                                </td>
                                <td>
...
<!-- ALERTA DE LOS CLIENTES QUE TIENEN DEUDA -->
<div id="cliente_deuda" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog w-porc-60">
        <div class="modal-content">
            <form id="formFormasPago" method="POST">
                <div class="modal-header text-center">
                ...
                <div class="modal-body panel panel-default">
                    <div class="row">
                        <div id="overflow_pantalla" class="col-12 mt-3">
                            <table class="table table-hover table-sm table-striped" id="" style="margin-bottom: 5px;">
                                <thead id="cabecera">
                                    <tr>
                                        <th>RUC</th>
                                        <th>RAZON SOCIAL</th>
                ...
                <div class="modal-footer">
                    <div id="row" class="col-sm-2 col-md-2 col-lg-2">
                        <button id="proceder_aceptacion" type="button" class="btn btn-sm btn-success">Proceder con la operacion de todos modos</button>
                        <button id="cancelar_aceptacion" type="button" class="btn btn-sm btn-danger">Cancelar la operacion</button>
                    </div>
                        <div id="" class="col-sm-2 col-md-2 col-2"
                            ...
                            <div id="mensajeEmergente">Los cientes de esta tabla tienen una deuda pendiente que pagar, usted decide si quiere realizar la operacion o no.</div>
                        </div>
                </div>
            </form>
        </div>
...
<!-- FIN ALERTA DE LOS CLIENTES QUE TIENEN DEUDA -->
...
<script>
    /* PRIMARY */
        ...
        let boton_acept =document.getElementById("proceder_aceptacion");

        boton_acept.addEventListener("click", function(){
            $("#cliente_deuda").modal("hide");
            $("#addItems").click();
        })

        let boton_cancelar =document.getElementById("cancelar_aceptacion");
        ...
        function validarClienteDeudor(ruc_cliente){

            let ruc_cliente_deuda = document.getElementById("ruc_cliente").value;
            
            $.ajax({
                url: "<?php echo base_url(); ?>index.php/tesoreria/cuentas/getEstadosPagosCuotas/",
                type: "POST",
                data: {
                    ruc_cliente_deuda: ruc_cliente_deuda
                },
                dataType: "json",
                success: function (data) {
                    ...
                    $("#table_body_deuda").empty();
                    if (data !== null) {
                        $("#cliente_deuda").modal("show");
                        // Itera sobre los datos y agrega filas a la tabla
                        $.each(data, function (index, item) {

                            console.log(item);

                            let tipo_documento = <?php echo json_encode($tipo_docu); ?>;

                            let tipo_operacion = <?php echo json_encode($tipo_oper); ?>;

                            //aqui se valioda los tipos de operacion y tipo de documentos para luego llenar en la tabla.
                            if (item.CPC_TipoDocumento == tipo_documento && item.CPC_TipoOperacion == tipo_operacion) { 
                                // Crea una nueva fila
                                var newRow = $("<tr>");
                                // Agrega las celdas con los datos correspondientes
                                newRow.append("<td>" + item.rucDni + "</td>");
                                newRow.append("<td>" + item.nombre + "</td>");
                                newRow.append("<td>" + item.CUE_FechaOper + "</td>");
                                newRow.append("<td>" + item.CPC_FechaVencimiento + "</td>");
                                newRow.append("<td>" + item.CUE_Monto + ' ' + item.MONED_Simbolo + "</td>");

                                // Agrega la fila al cuerpo de la tabla
                                $("#table_body_deuda").append(newRow);   
                            } 
                        });
                    } 
                }
            });
            // $("#addItems").click();  cliente_deuda
            // $("#cliente_deuda").modal("show");
        }
        ...
        $(function () {
            ...
            $("#buscar_cliente").autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    get_obra(ui.item.codigo);
                    //AQUI SE LLAMA A LA FUNCION DONDE HACE CONSULTA PARA DETERMINAR SI UN CLIENTE ES UN DEUDOR O NO.
                    let valor_ruc = validarClienteDeudor(ui.item.ruc);
  
                    let item_valor = document.getElementById("addItems");

                    // console.log(valor_ruc)
                    // $("#addItems").click();
