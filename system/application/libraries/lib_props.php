...
class Lib_props
{
  ...
  public function comprobante_pdf_ticket($codigo, $enviarcorreo = false)
  {
    ...
    foreach ($detalle_comprobante as $indice => $valor) {
      ...
      $detaProductos = $detaProductos . '
                ...
                <td style="border-top:#D1D1D1 1mm solid; text-align:left; width:47%;">' . $nombre_producto .' '. $valor->COLOR_Nombre.' '.$valor->PROD_Talla .'</td>
    ...
    if (count($othersFormasP) > 0) {
      ...
      {
        $otherpaymentsHTML .= str_repeat('&nbsp;', 3).strtoupper($others->FORPAC_Descripcion).' - ['.$others->MONED_Simbolo.' '.number_format($others->monto, 2).']';
    ...
    $footerHTML = '
              ...
              <tr>
              <td><b>FORMA DE PAGO: </b>' . $otherpaymentsHTML . '</td>
              </tr>
  ...
  public function comprobante_pdf_a4($codigo, $flagPdf = 1, $enviarcorreo = false)
  {
    ...
    $othersFormasP = $this->ci->comprobante_formapago_model->getList($codigo);
      ...
      //ESTE CODIGO SE QUITO DEL CUERPO DEL BODY <td colspan=""style="width:15.5cm;">'; ESE ERA IGUAL QUE EL CODIGO COMENTADO
