...
class Comprobante extends Controller{
    ...
    public function __construct(){
        ...
        $this->load->library('lib_props'); //carga lista de vendedores
    ...
    # INICIO
                ...
                if ($aceptada["exito"] == true || $comprobante->CPC_TipoDocumento == 'N'){
                    ...
                    if ($comprobante->CPC_FlagMueveStock == '0') {
                        ...
                        if($this->db->trans_status() == false){
                            ...
                            # SI UNA CAJA ESTA ASOCIADA AL DOCUMENTO, REGISTRA EL MOVIMIENTO EN LA CAJA
                            #&& $comprobante->FORPAP_Codigo == 1 esa condicion estaba validando solo para el tipo de pago 
                            if ($comprobante->CAJA_Codigo != "" && $comprobante->CAJA_Codigo != NULL){
                                $cuenta = $this->cuentas_model->getCuentaComprobante($codigo);
                                $justification = ($comprobante->CPC_TipoOperacion == "V") ? "INGRESO POR VENTA " : "EGRESO POR COMPRA ";
                                $justification = $justification . $comprobante->CPC_Serie . " - " . $this->lib_props->getOrderNumeroSerie($comprobante->CPC_Numero);
        ...
        public function envioSunatFac($codigo, $datos_comprobante){
            ...
            if ($tipo_oper == 'C'){
                ...
                $nubecont           = $datos_comprobante->CPP_Nubecont; //nubecont;
                ...
                */
                ...
                $arrayData = array(
                    ...
                    "nubecont_tipo_de_venta_codigo"     => "${nubecont}"
    ...
    public function comprobante_insertar(){
        ...
        $nubecont       = $this->input->post('tipo_operacionN');
        ...
        if (count($formPagoFP) == 1) {
            $filter_tipo_pago = new stdClass();
            $filter_tipo_pago->CPP_Codigo = $comprobante;
            $filter_tipo_pago->FORPAP_Codigo = $f_pago;
            $filter_tipo_pago->MONED_Codigo = "1";
            $filter_tipo_pago->monto = $fp_monto;
            $filter_tipo_pago->compro_flag_FechaRegistro = date("Y-m-d H:i:s");
            $this->comprobante_model->insertFormaPago($filter_tipo_pago);
        }
        /*:::::::::: CUOTAS ::::::::::::::::*/
    ...
    public function comprobante_insertar_array(){
        ...
        //$filter->CAJA_Codigo        = ""; ya funciona con esta modificacion
        $filter->CAJA_Codigo      = $datos["caja"];
        ...
        $filter->FORPAP_Codigo = "0";
        $filter->FORPAP_Monto   = $datos["importetotal"];
                 
        //$filter->CPC_Pagado=$datos["monto_temp"];
        $filter->COMPP_Codigo = $compania;
        $filter->CPC_NumOperacion = $datos["num_operacion"];

        //COMIENZO DE LA INSERCION DE FORMA DE PAGO, 
        $condiciones_pago = $datos["condiciones_pago"];

        $comprobante = $this->comprobante_model->insertar_comprobante($filter);

        for ($i=0; $i <count($condiciones_pago) ; $i++) { 
            $formaPago = $datos["condiciones_pago"][$i]["tipo_pago"];
            $montoPago = $datos["condiciones_pago"][$i]["monto_pago"];

            $filter_tipo_pago = new stdClass();

            $filter_tipo_pago->CPP_Codigo = $comprobante;
            $filter_tipo_pago->FORPAP_Codigo = $formaPago;
            $filter_tipo_pago->MONED_Codigo = $formaPago;
            $filter_tipo_pago->monto = $montoPago;
            $filter_tipo_pago->compro_flag_FechaRegistro = date("Y-m-d H:i:s");
    ...
    public function comprobante_editar($codigo, $tipo_oper = 'V', $tipo_docu = 'F'){
        ...
        $nubecont           = $datos_comprobante[0]->CPP_Nubecont;
