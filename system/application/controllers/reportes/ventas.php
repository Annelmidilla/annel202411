...
#FIN GANANCIA
    ...
    public function descargarExcel($fechaini, $fechafin){
        ...
        ###########################################################################
            ...
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("F$lugar", "COD. PROD.");
            ...
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("L$lugar", "PRECIO COMPRA");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("M$lugar", "UTILIDAD");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("N$lugar", "ESTADO UTILIDAD");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("O$lugar", "N.A");
            $this->excel->getActiveSheet()->getStyle("A$lugar:O$lugar")->applyFromArray($estiloColumnasTitulo);
            ...
            if ($resumen != NULL){
                ...
                $total_utilidad = 0;
                foreach($resumen as $indice => $valor){
                    $suma_total_venta += $valor->CPDEC_Total;
                    $suma_total_compra +=$valor->PROD_UltimoCosto * $valor->CPDEC_Cantidad;


                    $total_utilidad = $total_utilidad + $valor->CPDEC_Total;
                    if ($valor->CPDEC_Total - ($valor->PROD_UltimoCosto * $valor->CPDEC_Cantidad) < 0) {
                        $estado_utilidad = "PERDIDA";
                    }else{
                        $estado_utilidad = "GANANCIA";
                    }
                    $fRegistro = explode(" ", $valor->CPC_FechaRegistro);
                    $this->excel->setActiveSheetIndex($hoja)
                    ->setCellValue("A$lugar", $valor->CPC_Fecha)
                    ...
                    ->setCellValue("E$lugar", $valor->PROD_Nombre)
                    // ->setCellValue("F$lugar", $valor->MARCC_CodigoUsuario)
                    ->setCellValue("F$lugar", $valor->PROD_CodigoInterno)
                    ->setCellValue("G$lugar", $valor->LOTC_Numero)
                    ->setCellValue("H$lugar", $valor->LOTC_FechaVencimiento)
                    ->setCellValue("I$lugar", $valor->CPDEC_Cantidad)
                    ->setCellValue("J$lugar", $valor->CPDEC_Pu_ConIgv)
                    ->setCellValue("K$lugar", $valor->CPDEC_Total)
                    ->setCellValue("L$lugar", $valor->PROD_UltimoCosto * $valor->CPDEC_Cantidad)
                    ->setCellValue("M$lugar", $valor->CPDEC_Total - ($valor->PROD_UltimoCosto * $valor->CPDEC_Cantidad))
                    ->setCellValue("N$lugar", $estado_utilidad)
                    ->setCellValue("O$lugar", 0);
                    if ($indice % 2 == 0)
                        $this->excel->getActiveSheet()->getStyle("A$lugar:O$lugar")->applyFromArray($estiloColumnasPar);
                    else
                        $this->excel->getActiveSheet()->getStyle("A$lugar:O$lugar")->applyFromArray($estiloColumnasImpar);
                ...
                $lugar++;
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("N$lugar", "SUMA TOTAL COMPRA");
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("O$lugar", "S/".$suma_total_compra);
                
                $lugar++;
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("N$lugar", "SUMA TOTAL VENTA");
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("O$lugar", "S/". $suma_total_venta);
                
                $lugar++;
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("N$lugar", "TOTAL GANANCIA");
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("O$lugar", "S/". ($suma_total_venta - $suma_total_compra));
...
public function registro_ventas_pdf2($tipo_oper, $tipo_doc = "F", $fecha1, $fecha2, $forma_pago="", $consolidado=0, $sucursal='') 
    ...
    public function CierreTicket($fechaapertura, $fechacierre, $vendedor, $caja, $cierre){
        $fechaActual = date("Y-m-d H:i:s");
        $Fcierre = ($fechacierre == '' || $fechacierre == 'null') ? $fechaActual : $fechacierre;

       $empresa = $this->somevar['empresa'] = $this->session->userdata('empresa');
       $establecimiento =  $this->somevar['compania'] = $this->session->userdata('compania');
       $datosEmpresa =  $this->empresa_model->obtener_datosEmpresa($empresa);
       $razonSocial = $datosEmpresa[0]->EMPRC_RazonSocial;
       $cajaInfo = $this->caja_model->getcierre($cierre);
       ...
       $cajaChica = $cajaInfo[0]['CACIER_CAJACHICA'];
       ...
       if($monedacod == '1'){
        ...
        for ($i = 0; $i < count($otrosMov); $i++) {
            if($otrosMov[$i]["forma_pago"]== 1){
                ...
                $Movimientos = $movIngreso - $movEgreso;
            }
                
        }

       
       $medidas = array(80, 500);
       $pdf = new TCPDF('P', 'mm', $medidas, true, 'UTF-8', false);
       $fecha = date("Y-m-d H:i:s");

        $pdf->SetCreator('MiAplicacion');
        $pdf->SetAuthor('MiNombre');
        $pdf->SetTitle('Ticket de Cierre de Caja');
        ...
        $cabeceraHTML .= '
            <tr>
            <td style="font-size: 8pt;"><b>CIERRE DE CAJA</b></td>
            </tr>
            <tr>
            <td style="font-size: 8pt;"><b>Fecha: '.$fecha.'</b></td>
            </tr>
            </table><hr>';
        $pdf->writeHTML($cabeceraHTML, true, false, true, '');

        $subCabecera = '
            <table>
            <tr>
            <td style="font-size: 8pt;"><b>Vendedor: '.$vendedor.'</b></td>
            </tr>
            <tr>
            <td style="font-size: 8pt;"><b>Fecha apertura: '.$fechaapertura.' </b></td>
            </tr>
            <tr>
            <td style="font-size: 8pt;"><b>Fecha cierre:   '.$fechacierre.'</b></td>
            </tr>
            </table>';
        $pdf->writeHTML($subCabecera, true, false, true, '');

        $cuadreHTML = '
        <table align="center">
        <tr> <td style="font-size: 10pt;"><b>CUADRE DE CAJA</b></td>
        </tr>
        </table>';
        $pdf->writeHTML($cuadreHTML, true, false, true, '');

        $originalVentas = $TotalFormasDepago - $MontoCRED + $cajaChica + $Movimientos;
       
        $cuadreBodyHTML = '
        <table>
        <tr>
        <td style="font-size: 8pt;"><b>Saldo inicial: '.$simbolo.''.$cajaChica.'</b></td>
        <td style="font-size: 8pt;"><b>Saldo final: '.$simbolo.''.$originalVentas.' </b></td>
        </tr>
        <tr>
        <td style="font-size: 8pt;"><b></b></td>
        </tr>
        </table>';
        $pdf->writeHTML($cuadreBodyHTML, true, false, true, '');

        $cuadreHTML2 = '
        <table align="center">
        <tr>
        <td style="font-size: 10pt;"><b>RESUMEN DE CAJA</b></td>
        </tr>
        </table>';

        $pdf->writeHTML($cuadreHTML2, true, false, true, '');
        ...
        for ($i = 0; $i < count($formaspago); $i++) {
            ...
            <table>
            <tr>
                <td style="font-size: 7pt;"><b>'.$formaspago[$i]['descripcion'].'</b></td>
                <td style="font-size: 7pt;"><b>('.$formaspago[$i]['cantidad'].')</b></td>
                <td style="font-size: 7pt;"><b>'.$simbolo.' + '.$formaspago[$i]['suma'].'</b></td>
            </tr>
            </table>';

            $pdf->writeHTML($tdPagos, true, false, true, '');
        ...
        <table><hr>
        <tr>
        <td  style="font-size: 8pt;"><b>Total(sin anuladas): '.$simbolo.''.$TotalFormasDepago.'</b></td>
        </tr>
        </table>
        ';
        $pdf->writeHTML($VentasSumaHTL, true, false, true, '');
        ...
        <table >
        <tr>
        <td align="center" style="font-size: 10pt;"><b>VENTAS ANULADAS</b></td>
        </tr>
        </table>';
        ...
        for ($i = 0; $i < count($ventasinfo); $i++) {
            ...
            if(empty($ventasinfo[$i]['nota']) && $ventasinfo[$i]['tipo_nota'] = 5){
                $tdCredito = '';

                $pdf->writeHTML($tdCredito, true, false, true, '');
            }else{
                $tdCredito2 = '
                <table>
                <tr>
                    <td style="font-size: 7pt;"><b>'.$ventasinfo[$i]['forma'].'</b></td>
                    <td style="font-size: 7pt;"><b>'.$ventasinfo[$i]['serie'].'-'.$ventasinfo[$i]['numero'].'</b></td>
                    <td style="font-size: 7pt;"><b> '.$simbolo.' - '.$ventasinfo[$i]['monto_credito'].'</b></td>
                </tr>
                </table>';
    
                $pdf->writeHTML($tdCredito2, true, false, true, '');
            }
        ...
        <table><hr>
        <tr>
        <td style="font-size: 8pt;"><b>Notas de credito('.$notas.'): '.$simbolo.' - '.$Totalnotas.'</b></td>
        </tr>
        <tr>
        <td style="font-size: 8pt;"><b>Anulados desde el sistema('.$TicketsAnu.'): '.$simbolo.' - '.$TotalAnu.'</b></td>
        </tr>
        </table>';

        $pdf->writeHTML($anulHTML, true, false, true, '');

        $otherMov = '
        <div></div>
        <table >
        <tr>
        <td align="center" style="font-size: 10pt;"><b>OTROS MOVIMIENTOS</b></td>
        </tr>
        </table>';

        $pdf->writeHTML($otherMov, true, false, true, '');
        ...
        for ($i = 0; $i < count($otrosMov); $i++) {
            ...
            <table>
            <tr>
                <td style="font-size: 7pt;"><b>'.$otrosMov[$i]['fecha'].'</b></td>
                <td style="font-size: 7pt;"><b>'.$otrosMov[$i]['justificacion'].'</b></td>
                <td style="font-size: 7pt;"><b>'.$otrosMov[$i]['monto'].'</b></td>
                <td style="font-size: 7pt;"><b>'.$movTipo.'</b></td>
            </tr>
            </table><hr>';

            $pdf->writeHTML($tdotrosmov, true, false, true, '');
       ...
       /*  $otroslHTML = '
        <table><hr>
        <tr>
        <td style="font-size: 8pt;"><b>Total(movimientos): '.$simbolo.''.$Movimientos.'</b></td>
        </tr>
        </table>';

        $pdf->writeHTML($otroslHTML, true, false, true, ''); */

    
        $Totalanulados = $notas + $TicketsAnu;
        ...
        <table>
        <tr>
        <td style="font-size: 10pt;"><b>Ventas: '.$simbolo.''.$originalVentas.'</b></td>
        </tr>
        <tr>
        <td style="font-size: 10pt;"><b>N° Tickets (aprobados): '. $Tickets.'</b></td>
        </tr>
        <tr>
        <td style="font-size: 10pt;"><b>N° Tickets (anulados): '. $Totalanulados.'</b></td>
        </tr>
        <tr>
        <td style="font-size: 10pt;"><b>Bruto: '.$simbolo.''.$brutototal.'</b></td>
        </tr>
        </table>
        ';
        $pdf->writeHTML($ventasHTML, true, false, true, '');

/*         $pdf->RoundedRect(6, 150, 70, 20, 1.50, '1111', '');
