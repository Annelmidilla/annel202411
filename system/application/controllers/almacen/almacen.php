...
class Almacen extends controller{
    ...
    public function obtener_stock_general_cajas(){
        $producto = $this->input->post('product');

        $consulta = $this->almacen_model->stock_general_por_cajas($producto);
       ...
       foreach ($consulta AS $datos => $value) {
         ...
         $result[] = array("resultado" => "success" , "empresa" => $empresa);
       }
       
       echo json_encode($result);
    }
    
   public function consultar_selector_compuesto(){
		$consultar = $this->input->post('consultar');
		// print_r($nombre) ;
        $data = array();
		$data['datos'] = $this->almacen_model->consultar_selector_compuesto($consultar);
