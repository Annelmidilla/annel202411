...
class Producto extends Controller{
    ...
    public function consultarProducto(){
        $consultaProducto = $this->producto_model->getConsultaProducto();
        echo ($consultaProducto);

    }

    // public function insertarCombo(){

    //     $insertarCombo = $this->producto_model->insertarCombo();
    // }
    ...
    public function productos($flagBS = 'B', $j = '0' ,$limpia=''){
        ...
        $data["tallas"] = $tallasPeruanas;
        $data["colores"] = $this->color_model->getTable();    

        $this->layout->view('almacen/producto_index', $data);
    }
    
    public function registrar_variantes() {
        // Obtener los datos de las variantes
        $variantes = $this->input->post('variantes');
        $id = $this->input->post("id");


        // Obtener los datos generales del producto
        $nvo_codigo = $this->input->post('nvo_codigo');
        $nvo_nombre = $this->input->post('nvo_nombre');
        $nvo_familia = $this->input->post('nvo_familia');
        $nvo_fabricante = $this->input->post('nvo_fabricante');
        $nvo_marca = $this->input->post('nvo_marca');
        $nvo_modelo = $this->input->post('nvo_modelo');
        $nvo_stockMin = $this->input->post('nvo_stockMin');
        $nvo_preciocosto = $this->input->post('nvo_preciocosto');
        $nvo_tipoAfectacion = $this->input->post('nvo_tipoAfectacion');
        ...
        $dataProducto = array(
            'PROD_FlagBienServicio' => 'B',
            'PROD_CodigoInterno' => '', // Aquí se formará el código interno
            'PROD_CodigoUsuario' => '',
            'PROD_CodigoOriginal' => '',
            'PROD_Nombre' => $nvo_nombre,
            'PROD_NombreCorto' => $nvo_nombre,
            'PROD_DescripcionBreve' => $nvo_nombre,
            'PROD_Modelo' => $nvo_modelo,
            'PROD_StockMinimo' => $nvo_stockMin,
            'PROD_UltimoCosto' => $nvo_preciocosto,
            'PROD_GenericoIndividual' => 'G',
            'PROD_FlagActivo' => '1',
            'PROD_FlagEstado' => '0',
            'AFECT_Codigo' => $nvo_tipoAfectacion,
            'FAMI_Codigo' => $nvo_familia,
            'FABRIP_Codigo' => $nvo_fabricante,
            'MARCP_Codigo' => $nvo_marca,
            'TIPPROD_Codigo' => 11,
            'PROD_Comentario' => NULL,
            'PROD_Imagen' => NULL,
            'PROD_EspecificacionPDF' => NULL,
            'LINP_Codigo' => NULL,
            'PROD_Presentacion' => '',
            'PROD_PadreCodigo' => NULL,
            'PROD_PartidaArancelaria' => NULL
        );
    
        // Insertar el producto en la base de datos
        $productoId = $this->producto_model->insertarNvoRegistro($dataProducto);
    
        // Si el producto se insertó correctamente, procedemos a insertar las variantes
        if ($productoId) {
            foreach ($variantes as $variante) {
                $codigoInterno = $nvo_codigo . '-' . $variante['color'] . '-' . $variante['talla'];
    
                $dataVariante = array(
                    'PROD_CodigoInterno' => $codigoInterno,
                    'PROD_CodigoUsuario' => $codigoInterno,
                    'PROD_Nombre' => $nvo_nombre,
                    'PROD_Modelo' => $nvo_modelo,
                    'PROD_StockMinimo' => $nvo_stockMin,
                    'PROD_UltimoCosto' => $nvo_preciocosto,
                    ...
                    'LINP_Codigo' => NULL,
                    'PROD_Presentacion' => '',
                    'PROD_PadreCodigo' => $productoId, // Relacionamos la variante con el producto principal
                    'PROD_PartidaArancelaria' => NULL,
                    'COLOR_id' => $variante['color'],
                    'PROD_Talla' => $variante['talla']
                );
    
                // Insertar la variante en la base de datos
               $update = $this->producto_model->insertarNvoRegistro($dataVariante);
               $id = $update;
            ...
            // Enviamos una respuesta de éxito (si deseas utilizar AJAX)
            $response = array('result' => 'success');
            echo json_encode($response);
            exit;
        } else {
            // Enviamos una respuesta de error (si deseas utilizar AJAX)
    ...
    public function registrar_img() {
        $upload_path = 'uploads/articulos/';
        ...
        if (!is_writable($upload_path)) {
            echo json_encode(array('success' => false, 'error' => 'La carpeta de carga no tiene permisos de escritura.'));
            return;
        }
        ...
        if (!isset($_FILES['file'])) {
            echo json_encode(array('success' => false, 'error' => 'Error en la subida del archivo.'));
            return;
        }
        ...
        //? extension(tipo imagen), ruta que va a la BD
        $file_extension = pathinfo($file['name'], PATHINFO_EXTENSION);
        $file_path= $upload_path . $file_name . '.' . $file_extension;
    
        // if (!in_array($file_type, $allowed_types)) {
        //     echo json_encode(array('success' => false, 'error' => 'Tipo de archivo no permitido.'));
        //     return;
        // }
    
        if ($file_size > $max_size) {
            echo json_encode(array('success' => false, 'error' => 'El archivo excede el tamaño máximo permitido.'));
            return;
        }
    
        //?MOVER A LA CARPETA DE DESTINO
        $destination = $upload_path . $file_name . '.png';
        if (!move_uploaded_file($file['tmp_name'], $destination)) {
            echo json_encode(array('success' => false, 'error' => 'No se pudo mover el archivo subido.'));
            return;
        }

        $this->guardar_path_img($file_name, $file_path);

        echo json_encode(array('success' => true, 'file_path' => $destination));
    }

   //?metodo para agregar la ruta de la imagen a la base de datos
    public function guardar_path_img($id, $path) {
        $data = array(
            'PROD_RutaImg' => $path
        );
    
        $this->producto_model->modificarRegistro($id, $data);
        ...
        if ( count($listado) > 0 ){
            foreach ($listado as $indice => $valor) {
                ...
                $lista[$indice] = array(
                    ...
                    $posDT++ => $valor->COLOR_id,
                    $posDT++ => $valor->PROD_Talla,
                    $posDT++ => '',
                    $posDT++ => $divNPC
                );
                ...
                $lista[$indice][$j] = "<button id='edicion' type='button' onclick='getProducto($valor->PROD_Codigo)' class='btn btn-default' title='Editar'>
    ...
    public function getProductoInfo(){
        ...
        $productsCombo = $this->producto_model->showCombo($productoResult[0]->PROD_Codigo);
        ...
        if ($productoResult != NULL){
            ...
            foreach ($productoResult as $i => $val) {
                $productoInfo = array(
                                        ...
                                        "id_tipo_producto" => $val->TIPPROD_Codigo,

                                        "talla_info" => $val->PROD_Talla,
                                        "color_info" => $val->COLOR_id,
                                        "imagen" => $val->PROD_RutaImg
            ...
            $productoComboInfo = array();
            foreach ($productsCombo as $i => $val) {
                
                $productoComboInfo[$i] = array(
                            "productCode" => $val->PROD_Codigo,
                            "productName"=> $val->PROD_Nombre,
                            "productComboCode"=> $val->PROD_Codigo_combo,
                            "productUserCode" => $val->PROD_CodigoUsuario,
                            "productCount" => $val->PROD_Cantidad
                                    );
            }

            $json = array("match" => true, "producto" => $productoInfo, "productoComboInfo" => $productoComboInfo, "unidades" => $unidades, "precios" => $precios );
        }
        else
    ...
    public function guardar_registro(){
        ...
        $dataCombo = $this->input->post("dataCombo");

        $id = $dataCombo["id"];
        $productComboCode = $this->input->post('productComboCode');

        $ListadoProductos = $this->input->post("ListadoProductos");
       
        $flagBS = $dataCombo["flagB_S"];
        $codigo = $dataCombo["nvo_codigo"];
        $nombre = $dataCombo["nvo_nombre"];
        $autocompleteCodigoSunat = $dataCombo["nvo_autocompleteCodigoSunat"];
        $codigoSunat = $dataCombo["nvo_codigoSunat"];
        $tipoAfectacion = $dataCombo["nvo_tipoAfectacion"];
        $descripcion = $dataCombo["nvo_descripcion"];
        $familia = $dataCombo["nvo_familia"];
        $fabricante = $dataCombo["nvo_fabricante"];
        $marca = $dataCombo["nvo_marca"];
        $modelo = $dataCombo["nvo_modelo"];
        $stockMin = $dataCombo["nvo_stockMin"];
        $nvo_preciocosto = $dataCombo["nvo_preciocosto"];
        $unidad = $dataCombo["nvo_unidad"];
        $pmoneda = $dataCombo["nvo_pmoneda"];
        $pcategoria = $dataCombo["nvo_pcategoria"];
        $precios = $dataCombo["precios"];
        $id_tipo_producto = $dataCombo["id_tipo_producto"];

        /* Datos zapatilla extra */
        $color = $dataCombo["nvo_talla"];
        $talla = $dataCombo["nvo_color"];
        ...
        # REGISTRO PRODUCTO
            ...
            $filter->TIPPROD_Codigo = $id_tipo_producto;
        ...
        $productsCombo = $this->producto_model->showCombo($productComboCode);

        $productCounter = 0;
        foreach ($ListadoProductos as $key => $value) {
            $productCode = $value["codproductoOriginal"];
            $productCount = $value["cantidad"];
            if($productsCombo[$productCounter]->PROD_Codigo_combo == null){
                $this->producto_model->insertarCombo($productCode, $productCount, $id);
            }else if($productsCombo[$productCounter]->PROD_Codigo_combo == $id && $productsCombo[$productCounter]->PROD_Codigo != $productCode ){
                $this->producto_model->insertarCombo($productCode, $productCount, $id);
            }
            $productCounter++;
        }
