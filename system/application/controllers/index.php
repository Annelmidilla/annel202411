...
class Index extends Controller{
	...
	public function pos( $msg = NULL ){
		...
		$data['cajas'] 				= $this->caja_model->getcajaUsuario();
		$data['cajas'] 				= $this->caja_model->getcajaUsuario();
		$cajas = $data['cajas'];
		...
		foreach($cajas as $valor){
			...
			$estado_caja =$valor["CAJA_FlagSituacion"];

			$caja_codigos[] = $caja_codigo;
		}	
		$data['cierres']            = $this->caja_model->getcierres($filter=null, $caja_codigos);
		$data['cuadres']            = $this->caja_model->getcuadres($filter=null, $caja_codigos);
	...
	public function getFavoritos()
	{
	    ...
	    foreach ($fav as $key => $value) 
	    {
	      ...
	      if ( count($datosUnidadProducto) > 0 ) {
		    ...
		    $datos=array("almacenProducto"=>1,"codigo"=>$value->PROD_Codigo,"codinterno"=>$value->PROD_CodigoInterno,"codunidad"=>$codunidad,"descripcion"=>$value->PROD_Nombre,"flagGenInd"=>"","img"=>$value->PROD_Imagen,"pventa"=>$pventa,"tipo_afectacion"=>$value->AFECT_Codigo,"value"=>$value->PROD_Nombre,"stock" => $value->ALMPROD_Stock, "talla_color"=>$value->COLOR_id);
