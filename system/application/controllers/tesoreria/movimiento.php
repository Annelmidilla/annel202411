...
class Movimiento extends Controller{
    ...
    #########################
        ...
        public function datatable_movimiento(){
            ...
            if (count($movimientosInfo) > 0) {
                foreach ($movimientosInfo as $indice => $valor) {
                    ...
                    $lista[] = array(
                        ++$dtPos => mysql_to_human($fecha[0]) . " A LAS " . $this->lib_props->formatHours($fecha[1]),
                        ++$dtPos => $valor->CAJA_Nombre,
                        ++$dtPos => $valor->tipCa_Descripcion,
                        ++$dtPos => $valor->MONED_Simbolo." ".$valor->CAJAMOV_Monto,
                        ++$dtPos => "<span class='bold $colors'>$valor->movimiento</span>",
                        ++$dtPos => $valor->CAJAMOV_Justificacion,
                        ++$dtPos => $btn_modal,
                        ++$dtPos => $btn_eliminar
                    );
                }
        ...
        public function balance_caja($value='')
        {
            ...
            if ($balance[0]->total_ingreso>0) {
                $total_ingreso  = $balance[0]->total_ingreso;
            }
            if ($balance[0]->total_salida>0) {
        ...
        public function guardar_registro(){
            ...
            $cierre = $this->input->post("id_cierre");
