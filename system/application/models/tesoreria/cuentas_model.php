...
class Cuentas_model extends Model {
    ...
    public function getDatosDeudas($ruc_cliente_deuda){
        $sql = "SELECT c.CUE_Codigo, c.CUE_TipoCuenta, c.DOCUP_Codigo, c.CUE_CodDocumento, c.MONED_Codigo, c.CUE_Monto, c.CUE_FechaOper, c.CUE_FlagEstadoPago, c.CUE_FechaCanc, c.COMPP_Codigo, c.CUE_FechaRegistro, c.CUE_FechaModificacion, c.CUE_FlagEstado,
                    ...
                    WHERE c.CUE_TipoCuenta = 1
                        AND c.CUE_FlagEstado = 1
                        AND c.COMPP_Codigo = 1
                        AND e.EMPRC_Ruc = '$ruc_cliente_deuda'
                        AND c.CUE_FlagEstadoPago IN ('V', 'A');
                ";
                //AND c.CUE_FlagEstadoPago IN ('C', 'V', 'A'); trae solo los que no pagaron nada y los que tiene un avance

                //CUE_FlagEstadoPago =  C = cancelado, V = no pago nada y A = tiene un avance de pago.
