...
class Almacenproducto_Model extends Model {
    ...
    public function obtener($almacen_ids, $producto_ids) {
        $compania = $this->compania;
        $this->db->select("*");
    
        if (!empty($almacen_ids) && !empty($producto_ids)) {
            $this->db->where_in("ALMAC_Codigo", $almacen_ids);
            $this->db->where_in("PROD_Codigo", $producto_ids);
        } elseif (!empty($almacen_ids)) {
            $this->db->where_in("ALMAC_Codigo", $almacen_ids);
        } elseif (!empty($producto_ids)) {
            $this->db->where_in("PROD_Codigo", $producto_ids);
        } else {
        }
    ...
    public function obtener_alamcen($codigoProd, $compania){
        
        $this->db->select('*');
        $this->db->from('cji_almacenproducto');
        $this->db->where_in('PROD_Codigo', $codigoProd);
    ...
    public function actualizar_stock($filter){
        ...
        $id = $this->db->update("cji_almacenproducto",$filter);
        return $id;
    }
