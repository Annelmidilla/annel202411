...
class Producto_model extends model{
    ...
    public function getConsultaProducto(){
        $sql = "SELECT COUNT(*) + 1 AS count FROM cji_producto";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result->count;
    }

    public function insertarCombo($productCode, $productCount, $id){
        $sql = "INSERT INTO cji_productocombo(PROD_Codigo, PROD_Cantidad, PROD_Codigo_combo) VALUES ('$productCode','$productCount','$id')";

        $query = $this->db->query($sql);
    }
        
    public function showCombo($comboCode){
        $sql = "SELECT p.PROD_Codigo, p.PROD_Nombre,pc.PROD_Codigo, pc.PROD_Codigo_combo, p.PROD_CodigoUsuario, pc.PROD_Cantidad
        FROM cji_producto as p
        INNER JOIN cji_productocombo as pc ON p.PROD_Codigo = pc.PROD_Codigo
        WHERE pc.PROD_Codigo_combo = '$comboCode'";
...
;
        ...
        $sql = "SELECT DISTINCT p.PROD_Codigo, p.*, marca.MARCP_Codigo, marca.MARCC_Descripcion as marca, col.COLOR_Nombre
                    FROM cji_productocompania pc
                        ...
                        LEFT join cji_color as col on col.COLOR_codInterno = p.COLOR_id
 ...
 /* codigo implementado por Ing. Christopher -- busqueda independiente para punto de venta*/ 
    public function buscar_producto_codigo($keyword, $flag, $compania){
     
        $where = "";
        if ( isset($keyword) && $keyword != "" && strlen($keyword) > 1 )
            $where .= "AND p.PROD_CodigoUsuario LIKE '%$keyword%'";
