...
class Almacen_Model extends Model{
    ...
    public function buscar($filter,$number_items='',$offset='')
    {
      ...
      public function stock_general_por_almacenes($producto){
        ...
        $this->db->where("prod.PROD_Codigo like '%".$producto."%'");
    ...
    public function stock_general_por_cajas($producto){
        $this->db->select_sum('pc.PROD_Cantidad', 'total');
        $this->db->from('cji_productocombo pc');
        $this->db->join('cji_almacenproducto ap', 'ap.PROD_Codigo = pc.PROD_Codigo_combo');
        $this->db->where('pc.PROD_Codigo', $producto);
	 ...
	 public function consultar_selector_compuesto($codigo){
        
        $sql = "SELECT TIPPROD_Codigo FROM cji_tipoproducto WHERE TIPPROD_Codigo = $codigo";
        $query = $this->db->query($sql);
