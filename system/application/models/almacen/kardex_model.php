...
public function verificarMovimiento($codKardex, $filter){
    ...
    public function registrar_kardex_transformaion($filter){
        foreach ($filter->PROD_Codigo as $index => $codigo) {
            $fecha = date('Y-m-d H:i:s');
            $data = array(
                'PROD_Codigo' => $codigo,
                'KARDC_Cantidad' => $filter->KARDC_Cantidad[$index],
                'KARD_Fecha' => $fecha,
                'KARDC_CodigoDoc' => $filter->KARDC_CodigoDoc,
                'DOCUP_Codigo' => $filter->DOCUP_Codigo,
                'PROD_Descripcion' => $filter->PROD_Descripcion,
                'KARDC_Serie' => $filter->KARDC_Serie,
                'KARDC_Numero' => $filter->KARDC_Numero,
                'KARDC_AlmacenDesc' => $filter->KARDC_AlmacenDesc,
                'MONED_Codigo' => $filter->MONED_Codigo,
                'KARDC_ProdAfectacion' => $filter->KARDC_ProdAfectacion,
                'KARDC_Costo' => $filter->KARDC_Costo,
                'KARDC_PrecioConIgv' => $filter->KARDC_PrecioConIgv,
                'KARDC_Subtotal' => $filter->KARDC_Subtotal,
                'KARDC_Total' => $filter->KARDC_Total,
                'COMPP_Codigo' => $filter->COMPP_Codigo,
                'TIPOMOVP_Codigo' => $filter->TIPOMOVP_Codigo,
                'LOTP_Codigo' => NULL,
                'KARDC_TipoIngreso' => $filter->KARDC_TipoIngreso,
                'Denominacion' => $filter->Denominacion,
                'NumDocRuc' => $filter->NumDocRuc,
                'ALMPROD_Codigo' => $filter->ALMPROD_Codigo,
                'CLIP_Codigo' => $filter->CLIP_Codigo,
                'PROVP_Codigo' => $filter->PROVP_Codigo,
                'USUA_Codigo' => $filter->USUA_Codigo,
                'KARDP_FlagEstado' => $filter->KARDP_FlagEstado
            );
    
            $this->db->insert('cji_kardex', $data);
        }
