...
$("#ruc_cliente").autocomplete({
	...
	// function search() //Búsqueda de Productos/Servicios
	// {
 	// 	 var parametros="term="+$("#search").val()+"&TipCli="+$("#TipCli").val()+"&tipo_oper=V&familia=&marca=&modelo"+"&moneda="+$("#moneda").val();
    //      $.ajax({
    //         data:parametros, 
    //         url:base_url+"index.php/maestros/temporaldetalle/buscar_producto_codigo/"+ $("#flagBS").val() + "/" + compania+"/"+$("#almacen").val(), 
    //         type:  'POST',
    //         beforeSend: function()
    //         {
    //             $("#results").html('<div class="loadingio-spinner-spin-lvpepxs6i1"><div class="ldio-0sdgcnk1lov"><div><div></div></div><div><div></div></div><div><div></div></div><div><div></div></div><div><div></div></div><div><div></div></div><div><div></div></div><div><div></div></div></div></div>');
	...
	//           					productName = this.value.split(" ");
	//           					productName = productName.slice(-2);
	//           					productName = productName.join(" ");
	          					
	// 			          	    $("#results").append('<div style="cursor:pointer;" class="col-md-2"><div class="card"><div class="card-body"><h6 class="card-title">'+productName+' [Stock: '+stock+']</h6><a href="#" onclick=agregar_producto('+this.codigo+') class="btn btn-primary"><i class="bx bx-cart-add"></i></a></div></div></div>');
	          		...
	          		else
	          		{
	          			...
	          			$.each(data, function(){
	          				...
	          				else
	          				{
								...
								$("#producto_s").val(this.value);

								var img_stock = base_url + "images/caja_1.png";
									...
									<div class="col-12 mb-3">
									  <div class="card">
										<div class="card-body d-flex align-items-center">
										  <div class="me-3">
											<h6 class="card-title">T:${this.talla} ${this.colorNombre} [${stock}]</h6>
											<p class="card-text">S./${venta}</p>
										  </div>
										  <div class="ms-auto">
											<a href="#" onclick="agregar_producto(${this.codigo})" class="btn btn-success me-2">+</a>
											<button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#StockModal" onclick="stockGeneral(${this.codigo})">
											  <img src="${img_stock}" width="20" height="20">
											</button>
										  </div>
										</div>
									  </div>
		...
		$.each(productos, function()
			{
				...
				html += '<tr>';
				html += '<th scope="row">'+this.Nombre+' '+this.color+' Talla: '+this.talla+'</th>';
				//precio
		...
		else
		{
			...
			if (indice2!=-1) 
			{
						...
						var igv_pp=parseFloat(precio_venta)-precio;
			 			...
			 			"Stock":0,
			 			"Total_igv":igv_pp,
						"color": productos_temp[indice2]["colorNombre"],
						"talla":  productos_temp[indice2]["talla"]
			 			};
...
function enviar()
{
	...
	var data=
	{
		...
		"igvtotal":$("#igv").html(),
		"importeBolsa":0,
		"importetotal":$("#total").html(),//este es el codigo que obtiene el total de laA ventas
		"salir":0,
		"codigo":"",
		"contiene_igv":1,
		"contacto":"",
		...
		"condiciones_pago":datos_function,
		"monto_temp":$("#monto_temp").val(),
		"num_operacion":$("#num_operacion").val(),
	}

	if(data.estado_caja == 0){
		Swal.fire({
			title: 'Aperture una caja',
			text: 'ninguna caja aperurada',
			icon: 'error',
			showCancelButton: true,
		});
		return;
	...
	let formaspagos = data.condiciones_pago;

	let sumaMontos = formaspagos.reduce((acumulador, formaPago) => acumulador + formaPago.monto_pago, 0);
	let importeTotal = data.importetotal;

	if(sumaMontos > importeTotal){
		let vuelto= sumaMontos - importeTotal;
		Swal.fire({
			title: '¡Venta realizada con Éxito!',
			text: 'Vuelto total :'+ ' S/'+vuelto,
			icon: 'success',
			showCancelButton: true,
	...
	if(sumaMontos < importeTotal){
		Swal.fire({
			title: 'El monto abonado es menor que el Total D:',
			text: 'Monto abonado:'+ ' S/'+sumaMontos+" de "+ "S/"+importeTotal,
			icon: 'warning',
			showCancelButton: true,
	...
	$.ajax({   //carga los datos
		...
		success:  function (response) 
		{
			...
			if (response>0) //si esta bien, limpia los campos llenados
			{
...
$("#modal_d_2").click(function(){
	$("#modal_addcliente_2").modal("show");
})
...
function cargar_productos(data)
{
		...
		$.each(data, function(){
			...
			else
			{
				...
				$("#results").append('<div style="cursor:pointer;" class="col-md-4"><div class="card"><div class="card-body"><h5 class="card-title"><i class="fa-solid fa-star" id="imagen'+this.codigo+'" onclick=fav_producto('+this.codigo+') data-img='+this.img+' style='+style+'></i><br>T:'+this.talla+' '+this.colorNombre+' ['+stock+']</h5><p class="card-text">S./'+venta+'</p><a href="#" onclick=agregar_producto('+this.codigo+') class="btn btn-primary">Agregar</a><button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#StockModal" onclick="stockGeneral(' + this.codigo + ')"> <img src= '+img_stock+' width="20" height="20" > </button></div></div></div></div>');
				productos_temp.push(this);
